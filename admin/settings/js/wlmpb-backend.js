...
(function( $ ) {
	...
	// Toggle subitems
	var $toogleItems = $( '.wlmpb-item-toggle' );
	...
	$toogleItems.on( 'click', function( event ) {
		...
		var $menuItem = $( this ).closest( '.top-menu' );
			$subItems = $menuItem.next( '.wlmpb-dynamic-subitems-wrap' );

			$( this ).toggleClass( 'wlmpb-item-toggle-active' );
...
function wlmpbCopyToClipboard(containerid) {
