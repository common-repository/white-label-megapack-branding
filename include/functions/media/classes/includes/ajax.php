 ...
 * @since 0.1.0
 */
function wlmpb_media_categories_ajax_query_attachments() {
	...
	$query = isset( $_REQUEST[ 'query' ] )
		? (array) sanitize_text_field($_REQUEST[ 'query' ])
		: array();
	...
	foreach ( $taxonomies as $taxonomy ) {
		if ( isset( $query[ $taxonomy ] ) ) {
			...
			if ( $query[ $taxonomy ] == 'no_category' ) {
				$all_terms_ids = wlmpb_media_categories_get_terms_values( 'ids' );
				array_push( $query[ 'tax_query' ], array(
					'taxonomy' => $taxonomy,
					'field'    => 'id',
					'terms'    => $all_terms_ids,
	...
	wp_send_json_success( $posts );
		...
		function wlmpb_sanitize_cat( $taxinput ) {
			return str_replace( array( '.', '-' ), array( '_DOT_', '_DASH_' ), $taxinput );
		}
 ...
 * @since 1.0.0
 */
function wlmpb_media_categories_ajax_update_attachment_taxonomies() {
	...
	$attachment_data = sanitize_file_name($_REQUEST['attachments'][ $id ]);
	...
	$taxinput =  wlmpb_sanitize_cat($_REQUEST['tax_input']);

	if ( isset( $attachment_data[ $taxonomy ] ) ) {
		wp_set_object_terms( $id, array_map( 'trim', preg_split( '/,+/', $attachment_data[ $taxonomy ] ) ), $taxonomy, false );
	} else if ( isset($_REQUEST['tax_input']) && isset( $_REQUEST['tax_input'][ $taxonomy ] ) ) {
		wp_set_object_terms( $id, $taxinput[ $taxonomy ], $taxonomy, false );
	} else {
