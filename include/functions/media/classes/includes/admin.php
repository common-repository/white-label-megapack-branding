 ...
 */
function wlmpb_media_categories_enqueue_admin_scripts() {
	global $pagenow;

	// Asset management
	$url = wlmpb_media_categories_get_plugin_url();
	...
	if ( wp_script_is( 'media-editor' ) && ( ( 'upload.php' == $pagenow ) || ( 'post.php' == $pagenow ) || ( 'post-new.php' == $pagenow ) ) ) {
		...
		$attachment_terms = wp_dropdown_categories( array(
			...
			'orderby'         => 'name',
			'show_count'      => true,
			'walker'          => new WLMPB_Media_Categories_Media_Grid_Walker(),
			'value'           => 'id',
			'echo'            => false
		) );
		...
		$no_categories  = __( 'No categories',  'white-label-megapack-branding' );
		$all_categories = __( 'All categories', 'white-label-megapack-branding' );
		...
		echo '/* <![CDATA[ */';
		echo 'var wlmpb_media_categories_taxonomies = {"' . 'media_category' . '":';
		...
		// Script
		wp_enqueue_script( 'white-label-megapack-branding-media-views', $url . 'assets/js/media-views.js', array( 'media-views' ), true );
	}

	// Styling
	wp_enqueue_style( 'white-label-megapack-branding-styling', $url . 'assets/css/admin.css', array() );
}
 ...
 * @since 0.1.0
 */
function wlmpb_media_categories_add_category_filter() {
	global $pagenow;
	...
	$selected_value = sanitize_text_field( $_GET[ 'term' ] )
		? sanitize_text_field($_GET[ 'term' ])
		: '';

	// Maybe looking for attachments with no terms
	if ( empty( $selected_value ) ) {
		$selected_value = sanitize_text_field( $_GET[ 'media_category' ] )
			? sanitize_text_field($_GET[ 'media_category' ])
			: '';
	} ?>

	<label for="media_category" class="screen-reader-text"><?php esc_html_e( 'Filter by Category', 'white-label-megapack-branding' ); ?></label>

	<?php

	$dropdown_options = wlmpb_media_categories_get_media_category_options( $selected_value );
 ...
 * @since 0.1.0
 */
function wlmpb_media_categories_restrict_manage_posts() {
	wlmpb_media_categories_add_category_filter();
}
 ...
 * @return string
 */
function wlmpb_media_categories_create_sendback_url() {
	...
	if ( isset( $_REQUEST[ 'mode' ] ) ) {
		$sMode = sanitize_text_field($_REQUEST[ 'mode' ]);
 ...
 * @since 0.1.0
 */
function wlmpb_media_categories_get_terms_values( $keys = 'ids' ) {
 ...
 */
function wlmpb_media_categories_is_action_bulk_toggle() {
 ...
 * @since 0.1.0
 */
function wlmpb_media_categories_custom_bulk_admin_footer() {
	global $post_type;

	// Make an array of post_type
	if ( is_array( $post_type ) ) {
		$wlmpb_media_categories_post_type = $post_type;
	} else {
		$wlmpb_media_categories_post_type   = array();
		$wlmpb_media_categories_post_type[] = $post_type;
	}

	// Check whether the post_type array contains attachment
	if ( in_array( 'attachment', $wlmpb_media_categories_post_type ) ) {
		...
		if ( ! empty( $media_terms ) && ! is_wp_error( $media_terms ) ) {
			...
			$wlmpb_media_categories_footer_script = "";
			$wlmpb_media_categories_footer_script .= " <script type=\"text/javascript\">";
			$wlmpb_media_categories_footer_script .= "jQuery(document).ready(function(){";

			// Add new hidden field to store the term_slug
			$wlmpb_media_categories_footer_script .= "jQuery('#posts-filter').prepend('<input type=\"hidden\" id=\"bulk_tax_cat\" name=\"bulk_tax_cat\" value=\"" . 'media_category' . "\" />');";
			$wlmpb_media_categories_footer_script .= "jQuery('#posts-filter').prepend('<input type=\"hidden\" id=\"bulk_tax_id\" name=\"bulk_tax_id\" value=\"\" />');";

			// Add new action to #bulk-action-selector-top
			$wlmpb_media_categories_footer_script .= "jQuery('#bulk-action-selector-top')";
			$wlmpb_media_categories_footer_script .= ".attr('onChange','" . $onChangeTxtTop . "')";
//				$wlmpb_media_categories_footer_script .=	".attr('onClick','" . $onChangeTxt . "')";
			$wlmpb_media_categories_footer_script .= ";";

			// Add new action to #bulk-action-selector-bottom
			$wlmpb_media_categories_footer_script .= "jQuery('#bulk-action-selector-bottom')";
			$wlmpb_media_categories_footer_script .= ".attr('onChange','" . $onChangeTxtBottom . "')";
//				$wlmpb_media_categories_footer_script .=	".attr('onClick','" . $onChangeTxt . "')";
			$wlmpb_media_categories_footer_script .= ";";

			// add bulk_actions for each category term
			foreach ( $media_terms as $term ) {
				$optionTxt = esc_js( __( 'Toggle', 'white-label-megapack-branding' ) . ' ' . $term->name );
				$wlmpb_media_categories_footer_script .= " jQuery('<option>').val('" . 'bulk_toggle' . "').attr('option_slug','" . $term->term_id . "').text('" . $optionTxt . "').appendTo(\"select[name='action']\");";
				$wlmpb_media_categories_footer_script .= " jQuery('<option>').val('" . 'bulk_toggle' . "').attr('option_slug','" . $term->term_id . "').text('" . $optionTxt . "').appendTo(\"select[name='action2']\");";
			}

			$wlmpb_media_categories_footer_script .= '});';
			$wlmpb_media_categories_footer_script .= '</script>';

			echo $wlmpb_media_categories_footer_script;
		}
	}
}
 ...
 * @since 0.1.0
 */
function wlmpb_media_categories_custom_bulk_action() {
	...
	if ( ! wlmpb_media_categories_is_action_bulk_toggle() ) {
		return;
	}
	...
	$sendback = wlmpb_media_categories_create_sendback_url();
 ...
 * @since 0.1.0
 */
function wlmpb_media_categories_custom_bulk_admin_notices() {
	global $pagenow;

	if ( ( 'upload.php' === $pagenow ) && ! empty( $_REQUEST[ 'bulk_toggled' ] ) ) {
		$message = sprintf( _n( 'Media bulk toggled.', '%s media bulk toggled.', $_REQUEST[ 'bulk_toggled' ], 'white-label-megapack-branding' ), number_format_i18n( sanitize_text_field($_REQUEST[ 'bulk_toggled' ] )) );
 ...
 * @since 0.1.0
 */
function wlmpb_media_categories_set_attachment_category( $post_ID ) {
