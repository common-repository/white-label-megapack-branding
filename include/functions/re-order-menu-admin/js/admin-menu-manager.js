  ...
  $(function () {
    ...
    $('#admin-menu-manager-edit').click(function (e) {
      ...
      if (isEditing) {
        ...
        buttonLabel.text(WlmpbAdminMenu.buttonSave);
        button.attr('title', WlmpbAdminMenu.buttonSave);
      } else {
        var data = {
          action   : 'wlmpbam_update_menu',
          adminMenu: WlmpbAdminMenu.adminMenu
        };

        $.post(ajaxurl, data, function () {
          buttonIcon.removeClass('dashicons-yes').addClass('dashicons-edit');
          buttonLabel.text(WlmpbAdminMenu.buttonSaving).fadeOut(1000, function () {
            $(this).text(WlmpbAdminMenu.buttonSaved)
                .fadeIn()
                .delay(1000)
                .fadeOut(50, function () {
                  $(this).text(WlmpbAdminMenu.buttonEdit).fadeIn(50);
                });
          });
        });
      }
    ...
    function changeMenu(e, ui) {
       ...
       */
      _.find(WlmpbAdminMenu.adminMenu, function (value, index) {
      ...
      if (oldItem) {
        oldIcon = WlmpbAdminMenu.adminMenu[oldItem[0]][6];

        if (oldItem.length === 1) {
          item = WlmpbAdminMenu.adminMenu[oldItem[0]];
          WlmpbAdminMenu.adminMenu.splice(oldItem[0], 1);
        } else if (oldItem.length === 2) {
          item = WlmpbAdminMenu.adminMenu[oldItem[0]][7][oldItem[1]];
          WlmpbAdminMenu.adminMenu[oldItem[0]][7].splice(oldItem[1], 1);
        }
      }
      ...
      if (currentPosition.length === 1) {
        ...
        item[5] = item[5] ? item[5] : (!!oldItem ? WlmpbAdminMenu.adminMenu[oldItem[0]][5] : '');
        item[6] = oldIcon ? oldIcon : 'dashicons-admin-generic';
        WlmpbAdminMenu.adminMenu.splice(currentPosition[0], 0, item);
      } else if (currentPosition.length === 2) {
        item[4] = '';

        if (WlmpbAdminMenu.adminMenu[currentPosition[0]][7].length > 0) {
          WlmpbAdminMenu.adminMenu[currentPosition[0]][7].splice(currentPosition[1], 0, item);
        } else {
          // This means the menu item hasn't had any children before.
          WlmpbAdminMenu.adminMenu[currentPosition[0]][7].push(item);
        }
      }
